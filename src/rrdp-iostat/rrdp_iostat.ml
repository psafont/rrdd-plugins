...
module Iostat = struct
  ...
  let get_unsafe (devs : string list) : (string * t) list =
    ...
    *)
    let cmdstring = Printf.sprintf "/usr/bin/iostat -ydx %s 1 1" dev_str in (* display 1 device report after a second *)
...
module Stat = struct
  ...
  let get_unsafe_dev (dev : string) : t =
    ...
    let get_stats_from filename =
      Unixext.string_of_file (path_root / filename) |> Utils.cut |> List.map Int64.of_string
    in
    let hw_sector_size =
      Unixext.string_of_file (path_root / "queue/hw_sector_size") |> Int64.of_string
    in
...
module Stats_value = struct
      ...
      let avg_reqs_completed_last_five_secs get_reqs_completed get_total_ticks =
        last_s3 |> Option.fold ~none:0L (* we don't have data from 5 secs ago, so give up *)
                               ~some:(fun last_s3 ->
                                       ...
                                       if num_reqs_in_last_five_secs = 0L then 0L else num_ticks_in_last_5_secs /// num_reqs_in_last_five_secs
                                     )
      in
      {
  ...
  let make_ds ~owner ~name ~key_format (value : t) =
    ...
    ; { name = key_format "iowait"
      ...
      ; value = Rrd.VT_Float value.iowait; ty = Rrd.Absolute
      ; units = "s/s"; min = 0.
      }
    ; { name = key_format "inflight"
      ; description = "Number of I/O requests currently in flight"
      ; value = Rrd.VT_Int64 value.inflight; ty = Rrd.Gauge
      ; units = "requests"; min = 0.
      }
    ] in
...
module Iostats_value = struct
  ...
  let [@warning "-27"] make_ds  ~owner ~name ~key_format (value : t) =
    ...
    ; { name = (key_format "avgqu_sz"); description = "Average I/O queue size"
      ; value = Rrd.VT_Float value.avgqu_sz; ty = Rrd.Gauge; units = "requests"; min = 0.
      }
    ] in
    make_dss ~owner ~values
end
...
let gen_metrics () =
  ...
  let data_sources_low_mem_mode =
    ...
    in
    [[ Rrd.Host, Ds.ds_make ~name:"Tapdisks_in_low_memory_mode"
        ~description:("Number of tapdisks in low memory mode")
        ~value:(Rrd.VT_Int64 count)
        ~ty:Rrd.Absolute ~units:"count" ~min:0. ~default:true ()
    ]] in
